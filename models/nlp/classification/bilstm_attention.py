...
class BiLstmAttention(BaseClassifier):
    def __init__(self, config, vocab_size, word_vectors):
        # 父类初始化
        BaseClassifier.__init__(self, config, vocab_size, word_vectors)
        # 构建模型
        self.build_model()
        # 初始化保存模型的saver对象
        self.init_saver()
    ...
    def build_model(self):
        ...
        with tf.name_scope("embedding"):
            ...
            embedded_words = tf.nn.embedding_lookup(embedding_w, self.input_x)
    ...
    def _attention(self, H):
        ...
        # ----------最后归一注意力乘以V（即H）得出z score----------
        # ----------z score代表句子里隐层最受关注的向量值-----------
        # 利用求得的alpha的值对H进行加权求和，用矩阵运算直接操作
        z = tf.matmul(tf.transpose(H, [0, 2, 1]), tf.reshape(self.alpha, [-1, self.config["sequence_length"], 1]))

        # 将三维压缩成二维sequeezeR=[batch_size, hidden_size]
        sequeezeR = tf.squeeze(z)
